...
{
	public function getAvatarsByLinkId($link_id) 
	{
		$array = array();
		$statement = $this->db->prepare("SELECT * FROM avatars WHERE link_id = :link_id");
	...
	public function setAvatarCreate($user_id, $link_id, $avatar_image, $avatar_size_width, $avatar_size_height, $avatar_border_radius)
	{
		$statement = $this->db->prepare("SELECT * FROM avatars WHERE link_id = :link_id");
		$statement->bindValue(":link_id", $link_id);
		$statement->execute();
		if($statement->rowCount() == 0) 
		{
			$statement = $this->db->prepare("INSERT INTO avatars SET
				user_id = :user_id,
				link_id = :link_id,
	...
	public function setEnabledAvatar($link_id, $avatar_id, $avatar_is_enabled) 
	{
		$statement = $this->db->prepare("SELECT * FROM avatars WHERE avatar_id = :avatar_id AND link_id = :link_id");
		$statement->bindValue(":avatar_id", $avatar_id);
		$statement->bindValue(":link_id", $link_id);
		$statement->execute();
		if($statement->rowCount() > 0) 
		{
			$statement = $this->db->prepare("UPDATE avatars SET avatar_is_enabled = :avatar_is_enabled WHERE avatar_id = :avatar_id");
	...
	public function deleteAvatar($avatar_id, $block_id) 
	{
